...
packages <- unique(c(
  "cli", "ellmer", "irr", "jsonlite", "tidyverse",
  "rmarkdown", "devtools", "gridExtra",
  "rlang", "here", "ggplot2", "renv"
))
...
if (!"dsl" %in% rownames(installed.packages())) {
  library(devtools)
...
```{r}
#| vscode: {languageId: r}
# Install dsl package from the local path (for SANE users) if you haven't
if (!"dsl" %in% rownames(installed.packages())) {
  library(here)
